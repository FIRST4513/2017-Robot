...
void VisionUpdate::Execute() {
	...
	// -----------------------------------------------------
	// Step 1 Calculate alignment error with wall
	// -----------------------------------------------------
	// Robot rotated RIGHT of center = + Positive
	// Robot rotated LEFT of center = - Negative
	...
	tgtDist = TgtHt / (tan(CnvrtDegToRad(htAngle)));

	// --------------------------------------------------------------
	// Step 3 Calculate Distance to Wall and ShiftDist
	// --------------------------------------------------------------
	//angleX = (ConvertImagePosToAngle(COGX , 98) - tgtAlignError);				// 105 (Ctr pos. comp. for miss-aligned camera)
	angleX = ConvertImagePosToAngle(COGX , 98);				// 105 (Ctr pos. comp. for miss-aligned camera)
	angleF = angleX + tgtAlignError;
	distToWall = tgtDist * (cos(CnvrtDegToRad(angleF)));
	tgtShiftDist = tgtDist * (sin(CnvrtDegToRad(angleF)));

	// --------------------------------------------------------------
	//  Step 4 Calculate Target Tip Distance and Heading
	// --------------------------------------------------------------
	// CalcProbTipHdg(currHdg, tgtAlignError, tgtDist, angleX);	// tipHdg and tipDist is set in this function
	double sidee = distToWall - tgtDistFromWall;						// distance to tip from wall- decreased for greater accuracy
	...
	double angleToTip = CnvrtRadToDeg(atan(tgtShiftDist/sidee));		// tip angle = atan(rise/run)
	double newHdg;
	if ((currYaw < 30) && (currYaw > -30)){								// where in center position ( <0 =
		newHdg = angleToTip;
	}
	else if ((currYaw < -30) && (currYaw > -90)){						// were in right position
		newHdg = angleToTip - 60;
	}
	else if ((currYaw < 90) && (currYaw > 30)){							// were in left position
		newHdg = angleToTip + 60;
	}
	else {
		newHdg = 0;														// something is wrong go straight
	}
	...
	// -----------------------------------------------------
	// ------- This was used to calculate translation
